...
echo "" >> changelog.txt && \
echo "" >> changelog.txt && \
echo "### :package: [Download JAR](https://repo1.maven.org/maven2/com/erudika/para-email-ses/${ver}/para-email-ses-${ver}-shaded.jar)" >> changelog.txt && \
gh release create -F changelog.txt -t "v$ver" $ver && \
rm changelog.txt

docker build -t erudikaltd/para-email-ses:${ver} . && docker push erudikaltd/para-email-ses:${ver}
